class levelElevenScene extends Phaser.Scene {
    constructor() {
        super("leveleleven");
...
this.map.findObject('Hoodie Cashier', (obj) => {
  ...
  createPortal() {
    ...
    this.map.findObject('Entrance to Outside area of SE14 and SE2', (obj) => {
        
      this.portal = this.physics.add.image(obj.x, obj.y, 'portal2')
  });
  this.map.findObject('Entrance to REC', (obj) => {
...
// }
  ...
  // this.map = this.make.tilemap({ key: this._LEVELS[this._LEVEL] });
      ...
      this.map = this.make.tilemap({key: 'outdoor4'});
    ...
    loadNextMap() {
        ...
        if (!this.loadingLevel) {
            ...
            this.cameras.main.on( 'camerafadeoutcomplete', () => {
              if (this._LEVEL === 11) {
      this.bgMusic.destroy();
              this.scene.start('leveltwelve',{level: 12, levels: this._LEVELS, newGame: false});
            } 
          });
          this.loadingLevel = true;
          }
