class levelSevenScene extends Phaser.Scene {
    constructor() {
        super("levelseven");
  ...
  create() {
    ...
    this.physics.add.overlap(this.player.weapon, this.crow, this.createQuest.bind(this));
    // this.physics.add.overlap(this.player.weapon, this.raccoon, this.killenemy.bind(this));
...
pointerdown() {
  ...
  createQuest() {
    ...
    this.map.findObject('se14 front', (obj) => {
        
      this.portal2 = this.physics.add.image(obj.x, obj.y, 'portal2')
  });
  this.map.findObject('se2 outside 2 to rec outside', (obj) => {
    
    this.portal3 = this.physics.add.image(obj.x, obj.y, 'portal2')
});

this.quest = this.add.text(1200, 400, 'PORTALS ARE OPEN', { font: '"Press to See Quest"' });

this.physics.add.overlap(this.player, this.portal3, this.loadNextMap2.bind(this));
...
this.map.findObject('Hoodie Cashier', (obj) => {
  ...
  createPortal() {
    ...
    this.map.findObject('from se2 outside 1', (obj) => {
...
spawnMonster2() {
  this.map.findObject('raccoon', (obj) => {
    
    this.raccoon = this.spawns.create(obj.x, obj.y, 'raccoon');
    this.raccoon.setInteractive();
  this.raccoon.body.setCollideWorldBounds(true);
  this.raccoon.body.setImmovable();
...
moveEnemies () {
  ...
  // this.map = this.make.tilemap({ key: this._LEVELS[this._LEVEL] });
      ...
      this.map = this.make.tilemap({key: 'outdoor3'});
    ...
    loadNextMap2() {
      ...
      if (!this.loadingLevel) {
          ...
          this.cameras.main.on( 'camerafadeoutcomplete', () => {
            if (this._LEVEL === 7) {
           this.bgMusic.destroy();
            this.scene.start('leveleleven',{level: 11, levels: this._LEVELS, newGame: false});
          } 
        });
        this.loadingLevel = true;
        }
