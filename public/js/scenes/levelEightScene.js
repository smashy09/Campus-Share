class levelEightScene extends Phaser.Scene {
    constructor() {
        super("leveleight");
  ...
  create() {
    ...
    this.anims.create({
      ...
      this.physics.add.overlap(this.player, this.card, this.createQuest.bind(this));
     
    // this.movie.setVisible(false);
    
    this.quest = this.add.text(280, 1000, 'GRAB STUDENT CARD', { font: '"Press to See Quest"' });
...
pointerdown() {
  ...
  createCard() {
    this.map.findObject('key item student pass', (obj) => {
      
      this.card = this.physics.add.image(obj.x, obj.y + 75, 'scard').setInteractive();
      this.card.setImmovable();
  });
  ...
  createQuest() {
    ...
    this.map.findObject('portal entrance 2nd floor', (obj) => {
    
      this.portal2 = this.physics.add.image(obj.x, obj.y, 'portal2')
  });
  this.map.findObject('portal entrance basement', (obj) => {
    
    this.portal3 = this.physics.add.image(obj.x, obj.y, 'portal2')
});
this.quest2 = this.add.text(280, 1000, 'PORTALS OPEN', { font: '"Press to See Quest"' });
this.quest2.setScale(4);

this.physics.add.overlap(this.player, this.portal2, this.loadNextMap.bind(this));
this.physics.add.overlap(this.player, this.portal3, this.loadBottomMap.bind(this));
...
this.map.findObject('Hoodie Cashier', (obj) => {
  ...
  createPortal() {
    ...
    this.map.findObject('portal entrance', (obj) => {
...
// }
  ...
  // this.map = this.make.tilemap({ key: this._LEVELS[this._LEVEL] });
      ...
      this.map = this.make.tilemap({key: 'SE14'});
    ...
    loadBottomMap() {
        ...
        if (!this.loadingLevel) {
            ...
            this.cameras.main.on( 'camerafadeoutcomplete', () => {
              if (this._LEVEL === 8) {
      this.bgMusic.destroy();
              this.scene.start('levelten',{level: 10, levels: this._LEVELS, newGame: false});
            } 
          });
          this.loadingLevel = true;
          }
    ...
    loadNextMap() {
      ...
      if (!this.loadingLevel) {
          ...
          this.cameras.main.on( 'camerafadeoutcomplete', () => {
            if (this._LEVEL === 8) {
    this.bgMusic.destroy();
            this.scene.start('levelnine',{level: 9, levels: this._LEVELS, newGame: false});
          } 
        });
        this.loadingLevel = true;
        }
    ...
    // this.scene.restart({level: 'leveltwo', levels: this._LEVELS, newGame: false});
      if (!this.loadingLevel) {
        ...
        this.cameras.main.on( 'camerafadeoutcomplete', () => {
          if (this._LEVEL === 5) {
            this.bgMusic.destroy();
          this.scene.start('levelseven', {level: 7, levels: this._LEVELS, newGame: false});
        } 
      });
      this.loadingLevel = true;
      }
